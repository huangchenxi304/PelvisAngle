...
CLASSES = ["LFH1", "LFH2", "LFH3", "LFHCE", "LIPSS", "LIPTE", "LOPAC", "RFH1", "RFH2", "RFH3", "RFHCE", "RIPSS", "RIPTE", "ROPAC"]
...
def evaluate(input_path, output_path, phase,save_img=True, assigned=False, IS_DRAW_TEXT=False):
    ...
    for i, gt_p in enumerate(pbar):
        ...
        LCE_angle = angle_between_non_intersecting_lines(predict_points[5],predict_points[12],predict_points[3],predict_points[6])

        Ljdqxj = angle_between_non_intersecting_lines(predict_points[5],predict_points[12],predict_points[4],predict_points[6])

        Lsharp_angle = angle_between_non_intersecting_lines(predict_points[5],predict_points[12],predict_points[5],predict_points[6])
